...
const ProfilePosts = ({user}) => {
    ...
    const handlePostSubmit = async () => {
        ...
        try {
            const postData = {
                content: newPost,
                images: folderName ? [`/uploads/${folderName}`] : [],  // Append folder path if files are uploaded
                timestamp: 'Just now',  // Can be dynamically set
                location: 'Unknown',  // This can be dynamic based on user input or geolocation
                likes: 0,
                comments: 0,
                shares: 0,
                isLiked: false,
                isBookmarked: false,
                type: 'adoption', // Or dynamically set the post type
            };
            ...
            if (response.status === 200) {
                setPosts([postData, ...posts]);  // Update state with new post
                setNewPost('');  // Reset the new post textarea
                setFolderName('');  // Clear uploaded file folder path
            } else {
                    ...
                    <div className="flex-1">
                        ...
                        <div className="flex items-center justify-between mt-3">
                            ...
                            <button
                                ...
                                onClick={handlePostSubmit}  // Call the new submit handler
                ...
                <div key={post.id} className={`rounded-lg shadow-sm p-6 border ${getPostTypeColor(post.type)}`}>
                    ...
                    <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center space-x-3">
                            <div
                                className="w-10 h-10 bg-gradient-to-r from-blue-400 to-purple-500 rounded-full flex items-center justify-center text-white text-sm font-bold">
                                {user?.name?.charAt(0)}
                            </div>
                            <div>
                    ...
                    {/* Post Stats */}
                    <div
                        className="flex items-center justify-between text-sm text-gray-500 dark:text-gray-400 mb-3 pb-3 border-b border-gray-200 dark:border-gray-600">
                    ...
                    {post.comments > 0 && (
                        <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-600">
                            <div className="flex space-x-3">
                                <div
                                    className="w-8 h-8 bg-gradient-to-r from-green-400 to-blue-500 rounded-full flex items-center justify-center text-white text-xs font-bold">
                                    S
                                </div>
                                <div className="flex-1">
                                    <div className="bg-gray-100 dark:bg-gray-700 rounded-lg p-3">
                                        <p className="text-sm text-gray-900 dark:text-white">
                                            <span className="font-medium">Sarah Chen:</span> So adorable!
                                            Congratulations on the adoption! 🎉
                                        </p>
                                    </div>
                                    <div
                                        className="flex items-center mt-1 text-xs text-gray-500 dark:text-gray-400 space-x-4">
